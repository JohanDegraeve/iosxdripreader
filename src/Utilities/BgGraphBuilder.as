...
package Utilities
{
	...
	public class BgGraphBuilder
	{
		...
		public static function unitizedDeltaString(showUnit:Boolean,highGranularity:Boolean):String {
			...
			if (CommonSettings.getCommonSetting(CommonSettings.COMMON_SETTING_DO_MGDL) == "true") {
				return delta_sign + valueAsString + (showUnit ? (" " + ModelLocator.resourceManagerInstance.getString('settingsview','mgdl')):"");
			} else {
				return delta_sign + valueAsString + (showUnit ? (" " + ModelLocator.resourceManagerInstance.getString('settingsview','mmol')):"");
			}
		}
