	...
	public class Trace
	{
		...
		private static function getSaveStream():void {
			...
			if (fileName == "") {
				...
				BackgroundFetch.writeStringToFile(filePath, "UDID =  " + LocalSettings.getLocalSetting(LocalSettings.LOCAL_SETTING_UDID) + " (This is not the iOS UDID, it's a UDID generated by the app, unique to the device and the app and used to register at QuickBlox" +
					"\nEven after reinstallation of the app, it will still be the same value\n");
				BackgroundFetch.writeStringToFile(filePath, "Application version = " + ModelLocator.resourceManagerInstance.getString('general','version'));
